...
@router.post("/", response_model=Ubicacion, status_code=status.HTTP_201_CREATED, dependencies=[Depends(get_current_user)])
...
@router.get("/", response_model=List[Ubicacion], dependencies=[Depends(get_current_user)])
def get_ubicaciones_endpoint(
    service: UbicacionService = Depends(get_ubicacion_service)
):
    return service.get_all_ubicaciones()

@router.delete("/{id_ubicacion}", status_code=status.HTTP_200_OK, dependencies=[Depends(get_current_user)])
...
@router.put("/{id_ubicacion}", response_model=Ubicacion, status_code=status.HTTP_200_OK, dependencies=[Depends(get_current_user)])
